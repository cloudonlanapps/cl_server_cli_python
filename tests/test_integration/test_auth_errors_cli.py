...
class TestAuthErrorsCLI:
    ...
    def test_unauthenticated_store_read(
        ...
        assert "auth" in result.output.lower() or "unauthorized" in result.output.lower() or "401" in result.output
    ...
    def test_insufficient_permissions_admin(
        ...
        assert admin_create_result.exit_code == 0, f"Create failed: {admin_create_result.output}"
        ...
        assert "permission" in result.output.lower() or "forbidden" in result.output.lower() or "403" in result.output or "unauthorized" in result.output.lower()
