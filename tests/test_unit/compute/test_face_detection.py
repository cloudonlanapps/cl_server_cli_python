...
def test_detect_polling_mode(mock_compute_client, temp_image_file, completed_job):
    ...
    # Run command
    runner = CliRunner()
    result = runner.invoke(cli, ["compute", "face-detection", "detect", str(temp_image_file)])
...
def test_detect_watch_mode(mock_compute_client, temp_image_file, completed_job):
    ...
    # Run command
    runner = CliRunner()
    result = runner.invoke(cli, ["compute", "face-detection", "detect", "--watch", str(temp_image_file)])

    # Verify
    assert result.exit_code == 0
    mock_compute_client.face_detection.detect.assert_called_once()

def test_detect_missing_file(mock_compute_client):
    """Test face-detection detect with missing file."""
    runner = CliRunner()
    result = runner.invoke(cli, ["compute", "face-detection", "detect", "/nonexistent/file.jpg"])
